...
public class Variables_DownloadFile_BulkUpload {
	
	
	WebDriver driver;
	private WebDriverWait wait10;

	public Variables_DownloadFile_BulkUpload(WebDriver driver){
	...
	String BULK_UPLOAD_URL = "https://figbytes.biz/BulkUpload/BulkUpload.aspx?T=84";
	String sampleFileName = "VariableSampleData.xlsx";
	String gridFileName = "Variable Data.xlsx";
	...
	public void sampleFileDownload() throws InterruptedException {
		...
		if(this.BulkUploadButton.isDisplayed()) {
			ReportUtils.test.log(LogStatus.INFO, "Variables Bulk Upload button available");
			Const.logger.info("Variables Bulk Upload button available");				
			CommonMethods.clickOn(BulkUploadButton);	
			Thread.sleep(5000);
			if(driver.getCurrentUrl().contains(BULK_UPLOAD_URL)) {				
				ReportUtils.test.log(LogStatus.INFO, "Variables Bulk Upload Page exist");
				Const.logger.info("Variables Bulk Upload Page exist");
				...
				for(File file : totalFiles) {
					if(file.getName().equals(sampleFileName)) {
						ReportUtils.test.log(LogStatus.PASS, "Variables Sample File downloaded successfully : "+ file.getName());
						Const.logger.info("Variables Sample File downloaded successfully : "+ file.getName()); 							
						Thread.sleep(5000);						
				        file.delete();
	                ...
	                ReportUtils.test.log(LogStatus.FAIL,"Variables Bulk Upload Page load failed");
	                Const.logger.info("Variables Bulk Upload Page load failed");			
				}
	 		} else {
	            ReportUtils.test.log(LogStatus.FAIL,"Variables Bulk Upload Button is not available");
	            Const.logger.info("Variables Bulk Upload Button is not available");
	 		}
			}
			catch(Exception e){
	...
	public void gridFileDownload() throws InterruptedException {
		...
		if(this.DownloadGridFileButton.isDisplayed()) {
			ReportUtils.test.log(LogStatus.INFO, "Variables Download Data button available");
			Const.logger.info("Variables Download Data button available");					
			...
			for(File file : totalFiles) {
				if(file.getName().equals(gridFileName)) {
					ReportUtils.test.log(LogStatus.PASS, "Variables Grid File downloaded successfully : "+ file.getName());
					Const.logger.info("Variables Grid File downloaded successfully : "+ file.getName()); 							
					Thread.sleep(5000);						
			        file.delete();
	                ReportUtils.test.log(LogStatus.PASS,"File deleted successfully: "+ file.getName());
	                Const.logger.info("File deleted successfully: "+ file.getName());					        
	...
	String File = "VariablesCompleted"; //VariablesError
	...
	public void bulkUpload() throws InterruptedException {
		...
		ReportUtils.test.log(LogStatus.INFO, "Running Variables bulkUpload method.");	
		Const.logger.info("Running Variables bulkUpload method.");
		...
		Const.logger.info("Numbers of Variables data on page before bulk upload : " +getDataBeforeBulkUpload);
		ReportUtils.test.log(LogStatus.INFO, "Numbers of Variables data on page before bulk upload : " +getDataBeforeBulkUpload);
...
//					    	Thread.sleep(5000);
						...
						if(fileName.equals(historyPageFileName)) {
			                ...
			                if(Status.getText().equals("Completed")) {
			        			...
			        			Const.logger.info("Numbers of Variables data on page after bulk upload : " +getDataAfterBulkUpload);
			        			ReportUtils.test.log(LogStatus.INFO, "Numbers of Variables data on page after bulk upload : " +getDataAfterBulkUpload);
			        			...
			        			if(beforeAndSavedRecordsDataStr.equals(getDataAfterBulkUpload)) {
					                ReportUtils.test.log(LogStatus.INFO,"Variables data is matching, Sum of 'Before bulk upload data' and 'Saved records' : "+beforeAndSavedRecordsDataStr+" and After bulk upload: "+getDataAfterBulkUpload);
					                Const.logger.info("Variables data is matching, Sum of 'Before bulk upload data' and 'Saved records' : "+getDataBeforeBulkUpload+" and After bulk upload: "+getDataAfterBulkUpload);					        				
			        			}else {
					                ReportUtils.test.log(LogStatus.INFO,"Variables data not matched");
					                Const.logger.info("Variables data not matched");	
			        			}
