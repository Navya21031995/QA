...
public class ChilledWater_DownloadFile_BulkUpload {
	
	WebDriver driver;
	private WebDriverWait wait10;

	public ChilledWater_DownloadFile_BulkUpload(WebDriver driver){

	    this.wait10 = new WebDriverWait(driver, 60);
		this.driver=driver;		    
		PageFactory.initElements(driver, this);
	}
	
	String BULK_UPLOAD_URL = "https://qa.figbytes.biz/BulkUpload/BulkUpload.aspx?T=18";
	String sampleFileName = "testing.xlsx";
	String gridFileName = "ChilledWaterReport.xlsx";
	
	@FindBy(xpath=CommonLocators.MEGA_MENU_DATA)
	WebElement dataMenu;
	
	@FindBy(xpath="//ul[@class='col-sm-4 list-unstyled']//a[text()='"+Const.CHILLED_WATER_SUB_MENU+"']")
	WebElement ChilledWater;
	...
	public void sampleFileDownload() throws InterruptedException {
		...
		CommonMethods.clickOn(ChilledWater);
		Thread.sleep(5000);
		try {
		if(this.BulkUploadButton.isDisplayed()) {
			ReportUtils.test.log(LogStatus.INFO, "Chilled Water Bulk Upload button available");
			Const.logger.info("Chilled Water Bulk Upload button available");				
			CommonMethods.clickOn(BulkUploadButton);	
			Thread.sleep(5000);
			if(driver.getCurrentUrl().contains(BULK_UPLOAD_URL)) {				
				ReportUtils.test.log(LogStatus.INFO, "Chilled Water Bulk Upload Page exist");
				Const.logger.info("Chilled Water Bulk Upload Page exist");
				...
				for(File file : totalFiles) {
					if(file.getName().equals(sampleFileName)) {
						ReportUtils.test.log(LogStatus.PASS, "Chilled Water Sample File downloaded successfully : "+ file.getName());
						Const.logger.info("Chilled Water Sample File downloaded successfully : "+ file.getName()); 							
						Thread.sleep(5000);						
				        file.delete();
	                ...
	                ReportUtils.test.log(LogStatus.FAIL,"Chilled Water Bulk Upload Page load failed");
	                Const.logger.info("Chilled Water Bulk Upload Page load failed");			
				}
	 		} else {
	            ReportUtils.test.log(LogStatus.FAIL,"Chilled Water Bulk Upload Button is not available");
	            Const.logger.info("Chilled Water Bulk Upload Button is not available");
	 		}
			}
			catch(Exception e){
	...
	public void gridFileDownload() throws InterruptedException {
		...
		if(this.DownloadGridFileButton.isDisplayed()) {
			ReportUtils.test.log(LogStatus.INFO, "Chilled Water Download Data button available");
			Const.logger.info("Chilled Water Download Data button available");					
			...
			for(File file : totalFiles) {
				if(file.getName().equals(gridFileName)) {
					ReportUtils.test.log(LogStatus.PASS, "Chilled Water Grid File downloaded successfully : "+ file.getName());
					Const.logger.info("Chilled Water Grid File downloaded successfully : "+ file.getName()); 							
					Thread.sleep(5000);						
			        file.delete();
	                ReportUtils.test.log(LogStatus.PASS,"File deleted successfully: "+ file.getName());
	                Const.logger.info("File deleted successfully: "+ file.getName());					        
	...
	@FindBy(xpath="//select[@id='ctl00_ContentPlaceHolder1_gvMapping_ctl07_ddlExcelColumn']")
	WebElement unitNameDropdown;
	
	@FindBy(xpath="//select[@id='ctl00_ContentPlaceHolder1_gvMapping_ctl07_ddlExcelColumn']/option[contains(text(),'Unit Name (Col: 6)')]")
	WebElement unitName;
...
//	WebElement tag3;
	...
	@FindBy(xpath= CommonLocators.TABLE_TO_BE_DISPLAYED)
	WebElement tabletodisplay;
	
	public void bulkUpload() throws InterruptedException {
		
		ReportUtils.test.log(LogStatus.INFO, "Running Chilled Water bulkUpload method.");	
		Const.logger.info("Running Chilled Water bulkUpload method.");	
		CommonMethods.clickOn(dataMenu);			
		CommonMethods.clickOn(ChilledWater);
		Thread.sleep(5000);
		wait10.until(ExpectedConditions.visibilityOf(tabletodisplay));
		...
		Const.logger.info("Numbers of Chilled Water data on page before bulk upload : " +getDataBeforeBulkUpload);
		ReportUtils.test.log(LogStatus.INFO, "Numbers of Chilled Water data on page before bulk upload : " +getDataBeforeBulkUpload);
...
//				    	Thread.sleep(8000);
		                ...
		                Const.logger.info("Alert was not present");					    	
			                ...
			                CommonMethods.validateDataInAlert(driver);
					    	CommonMethods.AlertAccept(driver);
					    	Thread.sleep(5000);
...
//					    	Thread.sleep(5000);
						...
						if(fileName.equals(historyPageFileName)) {
			                ...
			                if(Status.getText().equals("Completed")) {
			        			...
			        			Const.logger.info("Numbers of Chilled Water data on page after bulk upload : " +getDataAfterBulkUpload);
			        			ReportUtils.test.log(LogStatus.INFO, "Numbers of Chilled Water data on page after bulk upload : " +getDataAfterBulkUpload);
			        			...
			        			if(beforeAndSavedRecordsDataStr.equals(getDataAfterBulkUpload)) {
					                ReportUtils.test.log(LogStatus.INFO,"Chilled Water data is matching, Sum of 'Before bulk upload data' and 'Saved records' : "+beforeAndSavedRecordsDataStr+" and After bulk upload: "+getDataAfterBulkUpload);
					                Const.logger.info("Chilled Water data is matching, Sum of 'Before bulk upload data' and 'Saved records' : "+getDataBeforeBulkUpload+" and After bulk upload: "+getDataAfterBulkUpload);					        				
			        			}else {
					                ReportUtils.test.log(LogStatus.INFO,"Chilled Water data not matched");
					                Const.logger.info("Chilled Water data not matched");	
			        			}
